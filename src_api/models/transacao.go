...
func EfetivarDebito(transacao Transacao, connPoll *pgxpool.Pool) error {
	...
	row := connPoll.QueryRow(context.Background(), `select (((limite + saldo) - $1) >= 0) podeEfetivar from clientes  where id=$2`, transacao.Valor, transacao.IdCliente)
	podeEfetivar := false
	...
	if podeEfetivar {
		...
		//Inserir Transacao
		_, err := connPoll.Exec(context.Background(), `INSERT INTO transacoes (idCliente, valor, tipo, descricao) VALUES ($1, $2, $3, $4) RETURNING id`, transacao.IdCliente, transacao.Valor, transacao.Tipo, transacao.Descricao)
		//Atualizar o saldo
		if err == nil {
			_, err = connPoll.Exec(context.Background(), `UPDATE clientes SET saldo= saldo + $1 WHERE id=$2`, transacao.Valor*-1, transacao.IdCliente)
//			if err != nil {
